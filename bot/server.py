...
def receive_event():
    ...
    conn.execute('''INSERT INTO events 
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)''',
        (datetime.now().strftime("%Y-%m-%d %H:%M:%S"), data['source_ip'], data.get('event_type', 'unknown'),
         severity, data.get('message', ''), data.get('server_name', 'unknown'),
         geo.get('country', '-'), geo.get('city', '-'), geo.get('provider', '-')))
    conn.commit()
    conn.close()
...
def receive_heartbeat():
    ...
    conn = get_db()
    conn.execute('''INSERT OR REPLACE INTO heartbeats (server_name, last_seen, is_online, os)
        VALUES (?, ?, 1, ?)''',
...
def get_pending_notifications():
    ...
    cursor = conn.execute('''SELECT * FROM events 
        ...
        ORDER BY timestamp DESC LIMIT 50''', (now,))
    events = [dict(row) for row in cursor.fetchall()]
    
    for e in events:
        cooldown = 300 if e['severity'] == 'CRITICAL' else (60 if e['severity'] == 'WARNING' else 0)
        if cooldown > 0:
            conn.execute('UPDATE events SET notified = 1, cooldown_until = datetime(?, ?) WHERE id = ?',
