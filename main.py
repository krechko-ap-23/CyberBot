...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_user.id != ADMIN_ID:
        ...
        return
    await update.message.reply_text("Нажми кнопку ниже, чтобы начать:", reply_markup=get_start_button(), parse_mode='HTML')
...
async def menu_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    cmd = query.data
    if cmd == "cmd_start":
        await query.edit_message_text("Бот готов, выберите действие:", reply_markup=get_main_menu(), parse_mode='HTML')
        return
    if cmd == "cmd_help":
...
async def check_heartbeats(context: ContextTypes.DEFAULT_TYPE):
    ...
    for s in cursor.fetchall():
        ...
        if diff > 60 and s['is_online'] == 1:
            ...
            conn.commit()
            await context.bot.send_message(chat_id=ADMIN_ID, text=f"{s['server_name']} оффлайн ({int(diff)}с)")
