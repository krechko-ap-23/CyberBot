...
from bot.commands import get_latest_events, get_alerts, get_top_ips, get_status_summary
...
def get_main_menu():
    keyboard = [
        ...
        [InlineKeyboardButton("⚠️ Тревоги", callback_data="cmd_alerts"), InlineKeyboardButton("🏆 Топ IP", callback_data="cmd_top")],
...
async def check_notifications(context: ContextTypes.DEFAULT_TYPE):
    ...
    for e in events:
        ...
        text = (
            f"{icon} {e['severity']} | {e['event_type']}\n"
            f"IP: {e['source_ip']}{geo_text}\n"
            f"Сервер: {e['server_name']}\n"
            f"{e['message'][:100]}..."
        )
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        "<b>Справка по меню:</b>\n\n"
        "<b>Логи</b> — Последние события из журнала безопасности.\n"
        "<b>Статус</b> — Статистика за 24ч и состояние серверов.\n"
        "<b>График</b> — Диаграмма атак с разбивкой по уровням угрозы.\n"
        "<b>ML: Обучить</b> — Обучение модели на данных за 7 дней.\n"
        "<b>ML: Проверить</b> — Поиск аномалий в последних событиях.\n"
        "<b>Тест дайджеста</b> — Мгновенная отправка суточного отчёта.\n"
        "<b>Остановить</b> — Завершение работы бота.\n\n"
        "<i>Кнопки в уведомлениях позволяют блокировать IP или смотреть историю атак.</i>"
    )
...
async def logs_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = f"Последние {len(events)} событий:\n" + "\n".join(
        [f"• {e['timestamp']} | {e['severity']} | {e['source_ip']}" for e in events]
    )
...
async def status_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if servers:
        text += "\nСерверы:\n" + "\n".join(
            [f"{'🟢' if s['is_online'] else '🔴'} {s['server_name']}" for s in servers]
        )
    await update.message.reply_text(text, reply_markup=get_main_menu())
...
async def ml_train_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("Готово!" if not err else err, reply_markup=get_main_menu())
...
async def menu_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    cmd = query.data
    if cmd == "cmd_start":
        await query.edit_message_text("Выберите действие:", reply_markup=get_main_menu())
        return
    if cmd == "cmd_help":
        text = (
            "<b>Справка по меню:</b>\n\n"
        ...
        await query.message.reply_text(text, reply_markup=get_main_menu(), parse_mode='HTML')
    elif cmd == "cmd_alerts":
        events = get_alerts(10)
        if not events:
            await query.message.reply_text("Тревог нет.", reply_markup=get_main_menu())
            return
        ...
        rows = get_top_ips(10)
        if not rows:
            await query.message.reply_text("Данных нет.", reply_markup=get_main_menu())
            return
        ...
        text = "Последние события:\n" + "\n".join(
            [f"• {e['timestamp']} | {e['severity']} | {e['source_ip']}" for e in events[:5]]
        ) if events else "Журнал пуст."
        await query.message.reply_text(text, reply_markup=get_main_menu())
    elif cmd == "cmd_status":
        events_24h, servers = get_status_summary()
        text = f"Событий за 24ч: {events_24h}"
        ...
        try:
            ...
            logger.error(f"Ошибка графика: {e}")
            await query.message.reply_text("Ошибка графика.", reply_markup=get_main_menu())
    elif cmd == "cmd_ml_train":
        await query.message.reply_text("Обучаю модель...", reply_markup=get_main_menu())
        _, err = train_model(days=7)
        await query.message.reply_text("Готово!" if not err else err, reply_markup=get_main_menu())
    elif cmd == "cmd_ml_check":
        await query.message.reply_text("Ищу аномалии...", reply_markup=get_main_menu())
        anomalies, err = detect_anomalies(days=1)
        if err or not anomalies:
            await query.message.reply_text("Аномалий не обнаружено." if not err else err, reply_markup=get_main_menu())
            return
        text = "Аномалии:\n" + "\n".join([f"• {a['source_ip']} | {a['reason']}" for a in anomalies[:3]])
        await query.message.reply_text(text, reply_markup=get_main_menu())
    elif cmd == "cmd_digest":
        await query.message.reply_text("Генерирую отчёт...", reply_markup=get_main_menu())
        await send_daily_digest(context)
    elif cmd == "cmd_stop":
...
async def check_heartbeats(context: ContextTypes.DEFAULT_TYPE):
    ...
    for s in cursor.fetchall():
        ...
        if diff > 60 and s['is_online'] == 1:
            ...
            conn.commit()
            await context.bot.send_message(chat_id=ADMIN_ID, text=f"🔴 {s['server_name']} недоступен ({int(diff)}с)")
    ...
    try:
        ...
        with open(digest['chart_path'], 'rb') as photo:
            await context.bot.send_photo(
                ...
                caption=digest['text'], reply_markup=get_main_menu(), parse_mode='HTML'
            )
    except Exception as e:
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "block":
        ...
        text = f"{ip}:\n" + "\n".join(
            [f"• {e['timestamp']} | {e['severity']}" for e in events[:5]]
        ) if events else "История пуста."
        await query.message.reply_text(text, reply_markup=get_main_menu())
